...
DATA_REFRESH_INTERVAL = 60  # 1 минута для автоматического обновления данных (уменьшено для более быстрого обновления)
...
if st.sidebar.button("🔄 Обновить данные", help="Очищает кэш и перезагружает данные из базы"):
    # Очищаем весь кэш Streamlit
    st.cache_data.clear()
    st.success("Кэш очищен! Данные будут перезагружены.")
    st.rerun()
...
# Основной контент
try:
    # Проверка подключения к БД и количества данных
    from app.database import engine
    try:
        count_query = "SELECT COUNT(*) as count FROM weather_data"
        count_result = pd.read_sql(count_query, engine)
        total_records = count_result.iloc[0]['count'] if not count_result.empty else 0
        
        # Показываем статус данных
        if total_records > 0:
            st.sidebar.success(f"✅ В базе: {total_records:,} записей")
        else:
            st.sidebar.warning("⚠️ База данных пуста")
    except Exception as e:
    ...
    if df.empty:
        ...
        st.info("💡 После загрузки данных нажмите кнопку '🔄 Обновить данные' в боковой панели")
    else:
...
# Автообновление и информация о кэше
col1, col2 = st.columns(2)
with col1:
    st.markdown(f"**Последнее обновление страницы:** {datetime.now().strftime('%H:%M:%S')}")
with col2:
    st.markdown(f"**Интервал автообновления кэша:** {DATA_REFRESH_INTERVAL} сек")
    
st.info("💡 **Совет:** После загрузки данных через API нажмите кнопку '🔄 Обновить данные' в боковой панели для немедленного обновления")
